...
rain = read.csv("Data_Inputs/SLR_Precip/Pluvio_Precip.csv", skip = 14) %>%
  mutate(Date = as.Date(Timestamp..UTC.08.00.)) %>% group_by(Date) %>%
  summarize(Precip = sum(Value)) %>%
  filter(Date >= start & Date <= end)

rmax_bar_height <- 0.15
precip_scale <- max_bar_height / max(rain$Precip, na.rm = TRUE)

ggplot() + 
  # Precip bars hanging from the top
  geom_rect(data = rain,
            aes(xmin = Date - 0.4, xmax = Date + 0.4,
                ymin = 0.5 - Precip * precip_scale, ymax = 0.5,
                fill = "Precipitation"),
            colour = NA) +
  ...
  scale_fill_manual(name = NULL, values = c("Precipitation" = "grey60")) +
  
  labs(title = "Daily Streamflow – Station 08LF027", x = NULL, y = expression(Discharge~(m^3/s))) +
  ...
  theme(
    ...
    axis.title.y.right = element_text(size = 11, margin = margin(l = 10), angle = 90),
  ...
  # Primary (discharge) axis + secondary (precip) axis
  scale_y_continuous(
